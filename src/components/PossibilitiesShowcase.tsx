          ...
          <div className="lg:col-span-8 bg-[#12142e]/80 backdrop-blur-2xl border border-[#7864ff]/25 rounded-3xl p-5 sm:p-7 shadow-[0_20px_50px_rgba(0,0,0,0.6),0_0_35px_rgba(85,69,255,0.18)] relative">
            ...
            <div className="relative rounded-2xl overflow-hidden border border-[#7864ff]/30 bg-[#070817] mb-5 group min-h-[320px] flex items-center justify-center">
              {activeGeneratedImage && viewMode === 'focused' ? (
                ...
                // 4-QUADRANT PREVIEW GRID (Variants A, B, C, D)
                <div className="w-full p-3 sm:p-4">
                  <div className="grid grid-cols-2 gap-3 sm:gap-4">
                    {[
                      ...
                      {
                        id: generatedHistory[1]?.id || DEFAULT_GENERATION_VARIANTS[1].id,
                        label: 'Variant B',
                        imageUrl: generatedHistory[1]?.imageUrl || DEFAULT_GENERATION_VARIANTS[1].imageUrl,
                        resolution: generatedHistory[1]
                          ? `${generatedHistory[1].aspectRatio || activeAspect} · ${generatedHistory[1].imageSize || activeImageSize}`
                          : DEFAULT_GENERATION_VARIANTS[1].resolution,
                        isGenerated: Boolean(generatedHistory[1]),
                      },
                      {
                        id: generatedHistory[2]?.id || DEFAULT_GENERATION_VARIANTS[2].id,
                        label: 'Variant C',
                        imageUrl: generatedHistory[2]?.imageUrl || DEFAULT_GENERATION_VARIANTS[2].imageUrl,
                        resolution: generatedHistory[2]
                          ? `${generatedHistory[2].aspectRatio || activeAspect} · ${generatedHistory[2].imageSize || activeImageSize}`
                          : DEFAULT_GENERATION_VARIANTS[2].resolution,
                        isGenerated: Boolean(generatedHistory[2]),
                      },
                      {
                        id: generatedHistory[3]?.id || DEFAULT_GENERATION_VARIANTS[3].id,
                        label: 'Variant D',
                        imageUrl: generatedHistory[3]?.imageUrl || DEFAULT_GENERATION_VARIANTS[3].imageUrl,
                        resolution: generatedHistory[3]
                          ? `${generatedHistory[3].aspectRatio || activeAspect} · ${generatedHistory[3].imageSize || activeImageSize}`
                          : DEFAULT_GENERATION_VARIANTS[3].resolution,
                        isGenerated: Boolean(generatedHistory[3]),
                      },
                    ].map((variant, index) => (
                      <div
                        key={variant.id}
                        ...
                        className={`group relative aspect-[4/3] rounded-2xl overflow-hidden border cursor-pointer transition-all duration-200 ${
                          selectedVariantIndex === index
                            ? 'border-[#7568ff] shadow-[0_0_20px_rgba(117,104,255,0.4)]'
                      ...
                      >
                        ...
                        />
                        <div className="absolute top-2.5 right-2.5 bg-[#050611]/80 backdrop-blur-md px-2 py-0.5 rounded-md text-[10px] font-semibold text-[#dedbff] border border-white/10 flex items-center gap-1">
                          {variant.isGenerated && <Sparkles className="w-2.5 h-2.5 text-[#8c78ff]" />}
                          <span>{variant.label}</span>
                        </div>

                        <div className="absolute inset-0 bg-gradient-to-t from-[#03040d]/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-between p-3">
                          ...
                          <button
                            ...
                            className="p-1 rounded-md bg-[#5545ff] text-white hover:bg-[#6859ff] transition-all cursor-pointer"
                          >
